...
void PlayerbotDruidAI::HealTarget(Unit &target, uint8 hp)
{
    ...
    else if (hp < 60 && LIFEBLOOM > 0 && !target.HasAura(LIFEBLOOM, 0) && ai->GetManaPercent() >= 28)
	{
		GetAI()->TellMaster("I'm casting lifebloom.");//<---changed
        ai->CastSpell(LIFEBLOOM, target);
	}
    else if (hp < 55 && REGROWTH > 0 && !target.HasAura(REGROWTH, 0) && !target.HasAura(REJUVENATION, 0) && ai->GetManaPercent() >= 33)
	{
        GetAI()->TellMaster("I'm casting regrowth.");//<---changed
        ai->CastSpell(REGROWTH, target);
	}
    else if (hp < 50 && SWIFTMEND > 0 && target.HasAura(REJUVENATION, 0) || target.HasAura(REGROWTH, 0) && ai->GetManaPercent() >= 16)
	{
		GetAI()->TellMaster("I'm casting swiftmend.");//<---changed
